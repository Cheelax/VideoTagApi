...
describe('hooks', function () {
    ...
    describe("Get Link", function () {
        it("should return all Link", async function () {
            ...
            const allVideo = await needle('get', 'http://localhost:49160/api/video/');
            allVideo.body.forEach(async function (element) {

                const rep = await needle('get', 'http://localhost:49160/api/video/' + element.id);
        ...
        it("should return no Link", async function () {
            ...
            const rep = await needle('get', 'http://localhost:49160/api/video/' + 1);
            var res = rep.body;
    ...
    describe("Delete Link", function () {
        it("should delete all Links", async function () {
            ...
            {
                var rep = await needle('get', 'http://localhost:49160/api/video/' + allVideoData[i].id);
                rep.body.tags = '[]';
                const video = {
                    ...rep.body
                };

                const res=await needle('put', 'http://localhost:49160/api/video/' + allVideoData[i].id, {
                    video
                }, {
                    json: true
                });
                var repDelete = await needle('get', 'http://localhost:49160/api/video/' + allVideoData[i].id);
    ...
    describe("Get videos by tag", function () {
        it("should return some Video", async function () {
            ...
            var repToUpdate = await needle('get', 'http://localhost:49160/api/videobytag/1');
...
async function PopulateDatabase() {
    ...
    {
        await needle("post", "http://localhost:49160/api/tag/", tagData.tags[i]);
